...
export class PulseValidator {
  ...
  private validateNode(node: ParsedNode, errors: ValidationError[]) {
    ...
    // Attributes
    if (node.attributes) {
      node.attributes.forEach(attr => this.validateAttribute(attr, errors));
    }
  ...
  private validateAttribute(attr: Attribute, errors: ValidationError[]) {
    // Event Format (camelCase)
    if (attr.name.startsWith('on')) {
      if (!/^on[A-Z]/.test(attr.name)) {
        const correct = 'on' + attr.name.slice(2).charAt(0).toUpperCase() + attr.name.slice(3);
        errors.push({
      ...
      if (!attr.value.startsWith('{') && !attr.value.startsWith('"{')) {
        errors.push({
          ...
          message: `Event handler must use curly braces: ${attr.name}={handler}`,
          code: 'PULSE010',
          severity: ValidationSeverity.Error
        });
      }
