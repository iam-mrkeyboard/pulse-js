...
export class PulseFormattingProvider {
  ...
  public format(document: TextDocument): TextEdit[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.length === 0) {
        ...
        if (consecutiveBlankLines > 1) {
          edits.push({
            range: Range.create(i, 0, i + 1, 0), // Delete entire line including newline
            newText: '',
          });
        }
        continue;
      ...
      if (currentIndent !== correctIndent) {
        edits.push({
          range: Range.create(i, 0, i, currentIndentLength),
      ...
      const openTagCount = (
        lineWithoutStrings.match(/<[a-zA-Z][a-zA-Z0-9\-]*[^/>]*>/g) || []
      ).length;
      const closeTagCount = (
        lineWithoutStrings.match(/<\/[a-zA-Z][a-zA-Z0-9\-]*>/g) || []
      ).length;
      const selfClosingCount = (
        lineWithoutStrings.match(/<[a-zA-Z][a-zA-Z0-9\-]*[^>]*\/>/g) || []
      ).length;
