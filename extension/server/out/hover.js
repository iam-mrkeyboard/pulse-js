...
class PulseHoverProvider {
    ...
    getHover(text, offset, tree, position) {
        if (!tree)
            return null;
        ...
        return (this.getPulseAPIHover(word) ||
...
\`\`\``,
        ...
        if (pulseAPIs[word]) {
            return {
                contents: { kind: node_1.MarkupKind.Markdown, value: pulseAPIs[word] },
            };
        }
        return null;
    }
    getJavaScriptBuiltInHover(word, inJSContext) {
        if (!inJSContext)
            return null;
        const jsBuiltIns = {
            // Array methods
...
\`\`\``,
        ...
        if (jsBuiltIns[word]) {
            return {
                contents: { kind: node_1.MarkupKind.Markdown, value: jsBuiltIns[word] },
            };
        }
        return null;
    }
    getVariableHover(word, text, inScriptBlock) {
        if (!inScriptBlock &&
        ...
        const signalPattern = new RegExp(`const\\s*\\[\\s*(${word})\\s*,\\s*set([A-Z][a-zA-Z0-9]*)\\s*\\]\\s*=\\s*createSignal\\(([^)]*)\\)`);
...
**Access:** \`${word}\` or \`${word}()\``,
        ...
        const constPattern = new RegExp(`(?:const|let|var)\\s+${word}\\s*=\\s*([^;]+)`);
...
**Value:** \`${value.trim()}\``,
        ...
        if (funcPattern.test(scriptContent)) {
            const funcMatch = scriptContent.match(new RegExp(`function\\s+${word}\\s*\\(([^)]*)\\)`));
...
**Usage:** \`${word}(${params})\``,
        ...
        const arrowPattern = new RegExp(`const\\s+${word}\\s*=\\s*\\(([^)]*)\\)\\s*=>`);
...
**Usage:** \`${word}()\``,
    ...
    getCSSPropertyHover(word, inCSSContext) {
        if (!inCSSContext)
            return null;
        const cssProperties = {
            // Display & Layout
...
\`\`\``,
    ...
    getCSSSelectorHover(word, beforeCursor, inStyleBlock) {
        if (!inStyleBlock)
            return null;
...
**Specificity:** 0,0,1`,
    ...
    getCSSValueHover(word, beforeCursor, inCSSContext) {
        if (!inCSSContext)
            return null;
        const cssValues = {
            // Display values
...
\`\`\``,
        ...
        if (pulsePrimitives[word]) {
            return {
                contents: { kind: node_1.MarkupKind.Markdown, value: pulsePrimitives[word] },
            };
        }
        if (/^[A-Z]/.test(word)) {
            return {
...
**Location:** \`components/${word}.pulse\``,
    ...
    getEventHandlerHover(word) {
        ...
        if (events[word]) {
            return {
                contents: { kind: node_1.MarkupKind.Markdown, value: events[word] },
            };
        }
        return null;
    }
