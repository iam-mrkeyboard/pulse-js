...
exports.PulseValidator = exports.ValidationSeverity = exports.GLOBALS = void 0;
...
(function (ValidationSeverity) {
    ValidationSeverity[ValidationSeverity["Error"] = 0] = "Error";
    ValidationSeverity[ValidationSeverity["Warning"] = 1] = "Warning";
    ValidationSeverity[ValidationSeverity["Information"] = 2] = "Information";
    ValidationSeverity[ValidationSeverity["Hint"] = 3] = "Hint";
...
class PulseValidator {
    ...
    checkScriptSyntax(text, root, errors) {
        const scriptNode = this.findScriptNode(root);
        if (!scriptNode || !scriptNode.children || scriptNode.children.length === 0)
            return;
    ...
    checkScriptScope(text, root, errors) {
        const scriptNode = this.findScriptNode(root);
        if (!scriptNode || !scriptNode.children || scriptNode.children.length === 0)
            return;
        const textChild = scriptNode.children.find(c => c.type === 'text');
        ...
        try {
            ...
            walk.ancestor(ast, {
                Identifier(node, ancestors) {
                    ...
                    if (!declared.has(name) && !exports.GLOBALS.has(name)) {
                        if (node.loc) {
                            const lines = textChild.content.split('\n');
                            let errorOffset = 0;
                            for (let i = 0; i < node.loc.start.line - 1; i++)
                                lines[i] && (errorOffset += lines[i].length + 1);
    ...
    validateAttribute(attr, name, errors) {
    ...
    checkDuplicateSlot(node, errors) {
    ...
    checkStateUsage(text, root, errors) {
    ...
    validateExpressions(node, stateVars, errors) {
    ...
    checkExpressionContent(expr, range, stateVars, errors) {
        let i = 0;
        while (i < expr.length) {
            const tag = 'state.';
    ...
    async checkImportExistence(text, currentFile, errors) {
        if (!this.fs)
            return;
        const imports = this.parser.extractImports(text);
