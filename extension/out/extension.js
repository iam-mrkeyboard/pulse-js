...
const PULSE_MODE = { language: 'pulse', scheme: 'file' };
// Decoration for state variables
let stateDecorationType;
let client;
function activate(context) {
    ...
    function updateDecorations(editor) {
        if (!editor || editor.document.languageId !== 'pulse')
            return;
        const text = editor.document.getText();
        const stateDecorations = [];
        ...
        while ((match = regex.exec(text))) {
            ...
            const decoration = {
                ...
                hoverMessage: new vscode.MarkdownString(`**Pulse Reactive State**\n\nVariable: \`${match[1]}\`\n\nChanges to this variable automatically update the UI.`),
            };
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('pulse.createComponent', createComponent));
    context.subscriptions.push(vscode.commands.registerCommand('pulse.createPage', createPage));
    // Status bar item
    ...
    client = new node_1.LanguageClient('pulseLanguageServer', 'Pulse Language Server', serverOptions, clientOptions);
    client.start();
}
...
async function createPage() {
    const name = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
            ...
            if (!/^[a-z]/.test(value))
                return 'Page name should be lowercase';
            return null;
        },
    });
    if (name) {
