...
export class ComponentCompiler {
  ...
  public async compile(filePath: string, content: string): Promise<string> {
    ...
    const hasListPrimitive = template.includes('<List');
    const hasShowPrimitive = template.includes('<Show');

    // Build module code
    let moduleCode = `
import { createSignal, createEffect } from '/runtime/core.js';
import { mountPrimitives as dom_mountPrimitives } from '/runtime/dom.js';
...
${imports.map(i => {
      ...
      // We need to handle default vs named. parser separates them?
      // ScriptParser returns ParsedImport { names: string[], source: string, isDefault: boolean }
      // Wait, ScriptParser interface (Step 4001) says: names, source, code, isDefault.
      // We can just use the 'code' directly? 
      // But ScriptParser cleans/removes imports from logic.
      // We should reconstruct OR use the code if it's clean.
      // Use manual reconstruction to be safe and clean.
      if (i.isDefault) {
...
`;
    ...
    if (hasState || functions.length > 0) {
      ...
      const { html, bindings } = this.templateTransformer.transform(template, [...stateVars, ...computedVars], imports.flatMap(i => i.names).filter(n => /^[A-Z]/.test(n)), declarations);
...
\n`;
      ...
      bindings.forEach((binding) => {
        if (binding.type === 'text') {
          ...
          moduleCode += `  createEffect(() => {\n`;
          moduleCode += `    const el = container.querySelector('[data-pulse-id="${binding.targetId}"]');\n`;
