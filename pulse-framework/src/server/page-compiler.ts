...
import { wrapHTML } from './html-wrapper';
import { getHMRScript } from './hmr-client';
...
export class PageCompiler {
  ...
  public async compile(filePath: string, content: string): Promise<string> {
    ...
    // Step 1: Parse with UnifiedParser (AST-based)
    const ast = this.unifiedParser.parse(content, filePath);

    let styles = '';
    let logic = '';
    let templateNode = null;
    let template = ''; // keep for fallback if needed, or reconstruct?
    // TemplateTransformer now takes AST root (ParsedNode).

    // Extract Logic
    if (ast.script.type === 'valid') {
      logic = ast.script.code;
    }

    // Extract Styles
    if (ast.styles.type === 'valid') {
      styles = ast.styles.code;
    }

    // Extract Template
    // UnifiedParser returns TemplateAST which has .ast (ParsedNode)
    if (ast.template.type === 'valid') {
      templateNode = ast.template.ast;
      template = ast.template.code; // Used for "hasListPrimitive" checks if regex used
    } else {
      // Create valid empty root
      templateNode = { type: 'element', tag: 'root', children: [] } as any;
    }

    const styleTag = styles ? `<style>${styles}</style>` : '';

    // Step 2: Analyze Script (using existing ScriptParser for now, or adapt UnifiedParser AST?)
    // Adapt: UnifiedParser returns ScriptAST with imports, signals, etc.
    // PageCompiler expects ScriptParseResult (imports, stateVars, functions..)
    // Let's rely on ScriptParser for Logic transformation *for now* to minimize changes,
    // OR map ScriptAST to ScriptParseResult.
    // Given ScriptParser is working, and UnifiedParser extracts `script` string:
    // We can just keep using `scriptParser.parse(logic)`.
    // Ideally we'd use ScriptAST, but `transformScript` logic depends on locations?
    // Let's stick to ScriptParser for the *logic* analysis for safety in this step,
    // but use UnifiedParser for the *splitting*.
    ...
    if (isInteractive) {
      if (!templateNode) throw new Error("Template Node missing");
      return this.compileInteractive(templateNode, template, scriptResult, styleTag, filePath);
    } else {
      // For static, we can just serialize the templateNode? 
      // Or just use the string if we have it?
      // UnifiedParser separates template.
  ...
  private compileInteractive(
    ...
    template: string, // Keep for legacy checks OR remove if we use AST for checks
    ...
    // Check for primitives using string check (fast) or AST walk?
    // String check is fine for now as a fallback/fast check.
    ...
    // ... (Imports logic for primitives unchanged) ...
    ...
    // PASS NODE TO TRANSFORMER
    const { html, bindings, templates } = this.templateTransformer.transform(templateNode, [...stateVarsWithSetters, ...computedVars], componentNames, declarations);
...
<script type="module">
import { createSignal, createEffect, createMemo } from 'pulse';
import { mountPrimitives as mountPrimitives_dom } from 'pulse/dom';
${primitiveImports.map(i => i.replace("'/runtime/primitives/list.js'", "'pulse/list'").replace("'/runtime/primitives/show.js'", "'pulse/show'")).join('\n')}
